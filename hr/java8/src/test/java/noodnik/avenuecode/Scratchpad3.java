...
public class Scratchpad3 {
    ...
    @Test
    public void sampleCase3b() {
        assertEquals(
            listOfLongs(121),
            maxSubsetSum(listOfInts(81))
        );
    }
    ...
    public void sampleTestCase3() {
        ...
        log("size of testData(%s)", testData.length);
        assertEquals(
            listOfLongs(expectedOutput),
            maxSubsetSum(listOfInts(testData))
        );
    ...
    public static List<Long> maxSubsetSum(List<Integer> k) {
        return k.stream().map(Scratchpad3::sumFactors).collect(Collectors.toList());
    }
    ...
    private static long sumFactors(int n) {
        ...
        // find list of primes <= 1/2 (n)
        final List<Integer> halfPrimes = findPrimes(n / 2);

        // collect set of multiplicands of each prime that evenly divides n
        ...
        for (int prime : halfPrimes) {
            ...
            factors.add(prime);
            for (int x : halfPrimes) {
                for (int i = n / prime; i > 1; i /= x) {
                    factors.add(i);
                    if (i % x != 0) {
                        break;
                    }
